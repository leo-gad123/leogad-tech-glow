...
-- Add policy to prevent unauthorized profile admin changes
...
WITH CHECK (
  ...
  -- Prevent changing is_admin unless you're the designated admin
  (
    OLD.is_admin = NEW.is_admin OR 
    (SELECT email FROM public.profiles WHERE user_id = auth.uid()) = 'hakizimanaleogad@gmail.com'
  )
);